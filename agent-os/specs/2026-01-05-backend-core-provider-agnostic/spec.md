...
Build a local-first, provider-agnostic email backend in Rust that handles account management, IMAP/SMTP protocols, MIME parsing, message threading, and sync orchestration to support a keyboard-first email client with offline capabilities.
...
- Support username/password authentication for IMAP/SMTP (defer OAuth to later phases)
...
**IMAP Fetch Engine with Multi-Folder Sync**

- Implement IMAP client using async-imap crate with tokio runtime
- Use UID-based incremental sync with UIDVALIDITY tracking per folder to detect server mailbox rebuilds
- Discover all folders via IMAP LIST command and store in folders table with auto-detected types (INBOX, Sent, Drafts, Trash, Archive)
- Sync INBOX every 5 minutes, other folders every 30 minutes using separate timers
- On initial account sync, fetch emails from last 30 days per folder, then background historical sync (30 days at a time going backward)
- Support parallel multi-folder sync using tokio::spawn for each folder, with INBOX prioritized
- Implement IDLE support for real-time INBOX notifications, triggering immediate sync on new message arrival
...
- Handle UIDVALIDITY changes by triggering full folder re-sync in background without blocking user
...
- User-friendly error messages: translate technical errors to actionable messages (e.g., "Failed to connect to IMAP server. Check your internet connection.")
- Fail fast and explicitly: return Result<T, E> for all fallible operations, never use unwrap() or expect() in production code
- Use specific error types: define custom error enums (SyncError, ImapError, SmtpError, DatabaseError) with thiserror crate
